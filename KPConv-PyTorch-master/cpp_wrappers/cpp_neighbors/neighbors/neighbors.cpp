								...
								float radius)
{
	...
	// 每次进行KDTree的建立，是对supports进行操作。supports 里面包含了很多 batch。每一个batch的尺寸用s_batches来记录
	...
	for (auto& p0 : queries)//当前循环代表，对于该batch中要以哪个点为中心进行查找
	{
		...
		// Find neighbors
		float query_pt[3] = { p0.x, p0.y, p0.z};//进行查找
		size_t nMatches = index->radiusSearch(query_pt, r2, all_inds_dists[i0], search_params);
		//nMatches 表示最多找了多少个匹配点，匹配规则：p0.x, p0.y, p0.z到 current_cloud 的点距离小于 r2 ，储存到 all_inds_dists 中

		// Update max count
		if (nMatches > max_count)//更新最大的点数
			max_count = nMatches;
	...
	// Reserve the memory
	neighbors_indices.resize(queries.size() * max_count);//扩充为最大的内存
	i0 = 0;
	sum_sb = 0;
	sum_qb = 0;
	b = 0;
	for (auto& inds_dists : all_inds_dists)
	{
		...
		for (int j = 0; j < max_count; j++)
		{
			if (j < inds_dists.size())
				...
				neighbors_indices[i0 * max_count + j] = supports.size();//按顺序储存，储存的是对于当前的queries的点，它在自己所属的batches中，半径为r的范围的点索引
			//超出范围的，直接把indices设置为数组的大小，也就是supports.size()，猜测这个值对于后面的处理来说是一个非法值
		}
		i0++;
	}
