...
void grid_subsampling(vector<PointXYZ>& original_points,
	...
	size_t fdim = original_features.size() / N;//不一定了 ModelNet40的是3
	size_t ldim = original_classes.size() / N;//应该是1

	// Limits of the cloud
	PointXYZ minCorner = min_point(original_points);
	PointXYZ maxCorner = max_point(original_points);
	PointXYZ originCorner = floor(minCorner * (1/sampleDl)) * sampleDl;//多取一点空间

	// Dimensions of the grid   z先不用算，算x,y方向上有多少个grid
    ...
    //降采样过程，将原始数据以sampleD1进行降采样，每个grid内的众多点取平均值作为代表该grid的点，对于feature的话也是类似
	for (auto& p : original_points)
	{
		...
		else if (use_classes)//classification 的情况下没有 label
	...
	for (auto& v : data)
	{
		...
		if (use_classes)
		{
		    for (int i = 0; i < ldim; i++)
		        ...
		        [](const pair<int, int>&a, const pair<int, int>&b){return a.second < b.second;})->first);//选择了当前grid中，众多点，哪一个类别数量最多
		}
	}
