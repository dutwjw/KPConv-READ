...
/*
Python Extend with C++ comment
PyErr_SetString set various type of exceptions 相当于故障检测机制
PyArray_FROM_OTF  http://numpy.org.cn/user/c-info/how-to-extend.html#%E8%BD%AC%E6%8D%A2%E4%BB%BB%E6%84%8F%E5%BA%8F%E5%88%97%E5%AF%B9%E8%B1%A1
Py_BuildValue() returns a tuple when its format string consists of zero or more format codes between parantheses.
Py_XDECREF Decrement the reference count for object
PyArray_NDIM return the number of the dimensions of the array
PyArray_DATA obtain the pointer to the data-buffer for the array
npy_intp Py_intptr_t (an integer that is the size of a pointer on the platform).
PyArray_SimpleNew Create a new uninitialized array of type, typenum, whose size in each of nd dimensions is given by the integer array, dims
Py_BuildValue Create a new value based on a format string similar to those accepted by the PyArg_Parse*() family of functions and a sequence of values
*/
...
static struct PyModuleDef moduledef = 
{
    ...
    "grid_subsampling",     // m_name  name of module
    module_docstring,       // m_doc   module documentation , may be NULL
...
{
    import_array();
	return PyModule_Create(&moduledef);//return a PyObject
}
/***固定写法***/
...
static PyObject* batch_subsampling(PyObject* self, PyObject* args, PyObject* keywds)
{
	...
	// Keywords containers
	static char* kwlist[] = { "points", "batches", "features", "classes", "sampleDl", "method", "max_p", "verbose", NULL };//NULL-terminated
	float sampleDl = 0.1;
...
static PyObject* cloud_subsampling(PyObject* self, PyObject* args, PyObject* keywds)
{
	...
	vector<float> original_features;
	vector<int> original_classes;
	original_points = vector<PointXYZ>((PointXYZ*)PyArray_DATA(points_array), (PointXYZ*)PyArray_DATA(points_array) + N);//传递参数
	if (use_feature)
		original_features = vector<float>((float*)PyArray_DATA(features_array), (float*)PyArray_DATA(features_array) + N * fdim);
	if (use_classes)
		original_classes = vector<int>((int*)PyArray_DATA(classes_array), (int*)PyArray_DATA(classes_array) + N * ldim);
