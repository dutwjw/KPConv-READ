...
class S3DISDataset(PointCloudDataset):
    ...
    def prepare_S3DIS_ply(self):
        ...
        for cloud_name in self.cloud_names:
            ...
            room_folders = [join(cloud_folder, room) for room in listdir(cloud_folder) if isdir(join(cloud_folder, room))]#判断是不是一个文件夹，以及列出文件的目录
            ...
            for i, room_folder in enumerate(room_folders):
                ...
                for object_name in listdir(join(room_folder, 'Annotations')):#Annotation 表示每个小物体的
