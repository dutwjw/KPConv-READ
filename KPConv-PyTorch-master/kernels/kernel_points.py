...
def kernel_point_optimization_debug(radius, num_points, num_kernels=1, dimension=3,
    ...
    kernel_points = np.random.rand(num_kernels * num_points - 1, dimension) * diameter0 - radius0#生成[-1,1]区间的3维点
    while (kernel_points.shape[0] < num_kernels * num_points):
        new_points = np.random.rand(num_kernels * num_points - 1, dimension) * diameter0 - radius0
        kernel_points = np.vstack((kernel_points, new_points))
        d2 = np.sum(np.power(kernel_points, 2), axis=1)#计算x^2+y^2+z^2
        kernel_points = kernel_points[d2 < 0.5 * radius0 * radius0, :]#确保随机生成的点在radius0范围内
    kernel_points = kernel_points[:num_kernels * num_points, :].reshape((num_kernels, num_points, -1))#还是只取前 num_kernel(100) * num_points 行

    # Optionnal fixing
    if fixed == 'center':
        kernel_points[:, 0, :] *= 0#对于每个num_kernels 的 num_points 首个元素设置为 0，也就是每一个核有一个中心点，其他的14个点随着他进行展开
    if fixed == 'verticals':
        kernel_points[:, :3, :] *= 0
        kernel_points[:, 1, -1] += 2 * radius0 / 3
        kernel_points[:, 2, -1] -= 2 * radius0 / 3
    ...
    saved_gradient_norms = np.zeros((10000, num_kernels))#应该是迭代10000次吧
    old_gradient_norms = np.zeros((num_kernels, num_points))
    for iter in range(10000):#这个循环主要算一个 gradients_norms 来设置Kernel的位置

        # Compute gradients
        # *****************

        # Derivative of the sum of potentials of all points 
        #kernel_points.shape is [num_kernels, num_points, 3]
        A = np.expand_dims(kernel_points, axis=2)#kernel_points.shape is [num_kernels, num_points, 1, 3]
        B = np.expand_dims(kernel_points, axis=1)#kernel_points.shape is [num_kernels, 1, num_points, 3]
        interd2 = np.sum(np.power(A - B, 2), axis=-1)#这个操作计算的是对于每个 num_kernels num_points到除了它以外的num_points的距离 interd2.shape[num_kernels, num_points, num_points]
        inter_grads = (A - B) / (np.power(np.expand_dims(interd2, -1), 3/2) + 1e-6)#类似于标准化的操作
        inter_grads = np.sum(inter_grads, axis=1) #inter_grads.shape[num_kernels, num_points, 3] 意义是 这个点到其他点的距离的总和  

        # Derivative of the radius potential
        circle_grads = 10*kernel_points#shape is [num_kernels, num_points, 3]

        # All gradients
        gradients = inter_grads + circle_grads#shape is [num_kernels, num_points, 3] 这个circle_grads 应该是 保证点不要离原点得太远 这个inter_grads 保证点之间不要离得太近
        ...
        gradients_norms = np.sqrt(np.sum(np.power(gradients, 2), axis=-1))#shape is [num_kernels, num_points] 
        saved_gradient_norms[iter, :] = np.max(gradients_norms, axis=1)#shape is [num_kernels] 意义 应该是对于每个num_kernels，num_points到其他的点的的距离总和的最大值
        ...
        moving_dists = np.minimum(moving_factor * gradients_norms, clip)# Gradient clipping value     clip = 0.05 * radius0  shape is gradients_norms[num_kernels, num_points] 

        # Fix central point
        if fixed == 'center':
            moving_dists[:, 0] = 0# 中心点不用动 
        if fixed == 'verticals':
            moving_dists[:, 0] = 0
        ...
        # moving factor decay
        moving_factor *= continuous_moving_decay#逐渐降低移动速度

    # Rescale radius to fit the wanted ratio of radius
    r = np.sqrt(np.sum(np.power(kernel_points, 2), axis=-1)) #shape is (num_kernels, num_points)
    kernel_points *= ratio / np.mean(r[:, 1:]) #除了第一原点，算所有点的平均值，相当于标准化
...
def load_kernels(radius, num_kpoints, dimension, fixed, lloyd=False):
    ...
    if not exists(kernel_file):
        if lloyd:
            ...
            # Find best candidate
            best_k = np.argmin(grad_norms[-1, :])#grad_norms shape is [10000, num_kernels]

            # Save points
            kernel_points = kernel_points[best_k, :, :]#原来如此，这是在100组点中找最好的那一组点，还是[num_kpoints,3]
    ...
    # Scale kernels
    kernel_points = radius * kernel_points #最后还是把尺度放回conv_radius
