...
def gather(x, idx, method=2):
    ...
    """
    #第一层 第一个参数的size 是 [n,1]  第二个是 [n,neighb_inds]
    if method == 0:
        return x[idx]
    elif method == 1:
        x = x.unsqueeze(1)
        ...
        for i, ni in enumerate(idx.size()[1:]):
            x = x.unsqueeze(i+1)#[n,1,1]
            new_s = list(x.size())#[n,1,1]
            new_s[i+1] = ni#[n,neighb_inds,1]
            x = x.expand(new_s)#copy 数据  [n,neighb_inds,1]
        n = len(idx.size())#2
        for i, di in enumerate(x.size()[n:]):#这里应该是对于feature每一个维度
            idx = idx.unsqueeze(i+n)#[n,neighb_inds,1]
            new_s = list(idx.size())
            new_s[i+n] = di#[n,neighb_inds,1]
            idx = idx.expand(new_s)#[n,neighb_inds,1]
        return x.gather(0, idx)#torch.gather    利用index来索引input特定位置的数值
    else:
...
class KPConv(nn.Module):
    ...
    def init_KP(self):
        ...
        K_points_numpy = load_kernels(self.radius,
                                      ...
                                      fixed=self.fixed_kernel_points)#返回一个numpy 数组 [K,3]
        return Parameter(torch.tensor(K_points_numpy, dtype=torch.float32),
                         requires_grad=False)#不需要梯度下降
    ...
    def forward(self, q_pts, s_pts, neighb_inds, x):
        ...
        neighbors = s_pts[neighb_inds, :] #s_pts shape [N,3]，这个可以提取最近邻的点

        # Center every neighborhood
        neighbors = neighbors - q_pts.unsqueeze(1)#类似于np的expand_dims q_pts shape 变成了 [N,1,3]  中心化坐标点
        ...
        # Get all difference matrices [n_points, n_neighbors, n_kpoints, dim] [点的总数，临近点，kernel point ， 维度3维]
        neighbors.unsqueeze_(2)
        differences = neighbors - deformed_K_points

        # Get the square distances [n_points, n_neighbors, n_kpoints] x^2+y^2+z^2算一起了
        ...
        if self.deformable:
            ...
            new_neighb_inds = neighb_inds#最近点索引
        ...
        if self.KP_influence == 'constant':
            ...
            all_weights = torch.clamp(1 - torch.sqrt(sq_distances) / self.KP_extent, min=0.0)#权重矩阵 all_weights [n_points, n_neighbors, n_kpoints]
            all_weights = torch.transpose(all_weights, 1, 2)# [n_points, n_kpoints, n_neighbors]
        ...
        x = torch.cat((x, torch.zeros_like(x[:1, :])), 0) #x 是 feature

        # Get the features of each neighborhood [n_points, n_neighbors, in_fdim]
        neighb_x = gather(x, new_neighb_inds)#第一个参数的size 是 [n,1]  第二个是 [n,neighb_inds] 这个函数的意思可以看这里 https://blog.csdn.net/cpluss/article/details/90260550
        #neighb_x 变成了 [n,neighb_inds,1]
        # Apply distance weights [n_points, n_kpoints, in_fdim]
        weighted_features = torch.matmul(all_weights, neighb_x)# [n_points, n_kpoints, n_neighbors]   [n_points,n_neighbors, in_fdim]
        ...
        weighted_features = weighted_features.permute((1, 0, 2))#[n_kpoints, n_points, in_fdim]
...
class SimpleBlock(nn.Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config):
        ...
        current_extent = radius * config.KP_extent / config.conv_radius#对于rigid 来说current_extent = KP_extent * first_subsampling

        # Get other parameters
        self.bn_momentum = config.batch_norm_momentum #0.99
        ...
        self.KPConv = KPConv(config.num_kernel_points,#15
                             config.in_points_dim,#3
                             in_dim,
                             out_dim // 2,#if 'simple' in block: in_dim = out_dim // 2
                             current_extent,
                             radius,
                             fixed_kernel_points=config.fixed_kernel_points,#center
    ...
    def forward(self, x, batch):# Save all block operations in a list of modules x = batch.features.clone().detach()

        if 'strided' in self.block_name:
            q_pts = batch.points[self.layer_ind + 1]#取出下一层的原始点
            s_pts = batch.points[self.layer_ind]#取出当前层的原始点   q_pts和s_pts应该是一样的
            neighb_inds = batch.pools[self.layer_ind]#对于采样后的点，基于stacked_points，找的r内的点的索引
        else:
            q_pts = batch.points[self.layer_ind]#取出当前层的 原始点
            s_pts = batch.points[self.layer_ind]#取出当前层的 点 
            neighb_inds = batch.neighbors[self.layer_ind]#对于 neighb_inds ，基于他自己找的r内的点的索引
