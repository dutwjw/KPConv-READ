...
class Config:#对于不同的数据集，都会继承这个类，对于Python的继承机制，子类继承后会覆盖父类中部分变量
    """
    ...
    def __init__(self):
        ...
        # Number of layers 查看 pool 和 strided 在网络结构中的个数
		# 推导式创建列表 循环创建 具有多个元素的列表，并且，按条件筛选部分元素。
        ...
        #   对于modelnet40这个数据集，deform_layers的结果是5个False的列表

        layer_blocks = []
        self.deform_layers = []
        arch = self.architecture#子类重写了父类的变量
        for block_i, block in enumerate(arch):

            # Get all blocks of the layer
			
            if not ('pool' in block or 'strided' in block or 'global' in block or 'upsample' in block):
                layer_blocks += [block]#相当于 攒 着
                continue
    ...
    def load(self, path):
        ...
        for line in lines:
            ...
            if len(line_info) > 2 and line_info[0] != '#':
                ...
                if line_info[2] == 'None':
                    setattr(self, line_info[0], None)#设置这个对象的属性
