...
class ModelTrainer:
        ...
        for epoch in range(config.max_epoch):
            ...
            for batch in training_loader:
                ...
                if config.grad_clip_norm > 0:
                    ...
                    torch.nn.utils.clip_grad_value_(net.parameters(), config.grad_clip_norm)#防止梯度太大
                self.optimizer.step()
                torch.cuda.synchronize(self.device)
                '''测试时间的方式，代码会同步cu的操作，等待gpu上的操作都完成了
				再继续成形end = time.time()'''
                t += [time.time()]
    ...
    def object_classification_validation(self, net, val_loader, config):
        ...
        softmax = torch.nn.Softmax(1)#在第一维上相加等于1

        # Initialize global prediction over all models
        if not hasattr(self, 'val_probs'):
            self.val_probs = np.zeros((val_loader.dataset.num_models, nc_model))#num_models 是指 文件的个数 2000 多个 nc_models 是类别的个数 40个
